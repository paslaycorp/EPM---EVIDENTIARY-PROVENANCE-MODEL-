"""Answer-space and resolution semantics for EPM."""
...
from .constraints import Constraint, ConstraintStatus, evaluate_constraint
from .sources import EpistemicSourceRecord, EpistemicSourceType, validate_source_record
...
def validate_answer_space(snapshot: AnswerSpaceSnapshot) -> AnswerSpaceValidation:
    ...
    if not snapshot.candidate_universe:
        return AnswerSpaceValidation(False, "CANDIDATES_REQUIRED", "Answer-space requires at least one candidate.")
    if len(set(snapshot.candidate_universe)) != len(snapshot.candidate_universe):
        return AnswerSpaceValidation(False, "CANDIDATES_NOT_UNIQUE", "Candidate universe must not duplicate alternatives.")
...
def recompute_with_constraints(
    ...
    before_set = set(snapshot.admissible_candidates)
    after_set = set(after)
    if after == snapshot.admissible_candidates:
        ...
        operation = (
            ...
            if set(snapshot.active_constraint_refs) - set(active)
...
def record_derivation(
    ...
    *,
    ...
    if candidate not in snapshot.admissible_candidates:
        return ResolutionOperationResult(snapshot, False, "CANDIDATE_NOT_ADMISSIBLE", "Derivation cannot select an excluded candidate.")
    ...
    return ResolutionOperationResult(updated, True, "DERIVATION_RECORDED", "Derivation was recorded without promoting resolution state.")
...
def assess_discriminator(
    ...
    if not current.issubset(set(discriminator.alternatives)):
        return DiscriminatorAssessment(False, "DISCRIMINATOR_INCOMPLETE_ALTERNATIVES", "Discriminator does not declare every admissible alternative.")
    covered: set[str] = set()
    for partition in discriminator.observation_partition.values():
        ...
        if len(relevant) > 1:
            return DiscriminatorAssessment(False, "DISCRIMINATOR_PARTIAL", "A discriminator outcome leaves multiple alternatives indistinguishable.")
    if covered != current:
    ...
    return DiscriminatorAssessment(True, "DISCRIMINATOR_SUFFICIENT", "Observation partition can distinguish every admissible alternative.")
...
def resolve_with_observation(
    ...
    if not source_check.valid or observation.source.source_type is not EpistemicSourceType.OBSERVATION:
        return ResolutionOperationResult(snapshot, False, "OBSERVATION_REQUIRED", "Stage 2 resolution requires a valid externally originated observation.")
    ...
    if len(selected) != 1:
        return ResolutionOperationResult(snapshot, False, "OBSERVATION_NOT_RESOLVING", "Observed discriminator value does not select exactly one admissible answer.")
    event = AnswerSpaceHistoryEvent(
    ...
    return ResolutionOperationResult(updated, True, "RESOLVED_BY_OBSERVATION", "A sufficient discriminator and external observation resolved the answer-space.")
...
def close_answer_space(
    ...
    if not closure.exhaustive_domain:
        return ResolutionOperationResult(snapshot, False, "EXHAUSTIVE_DOMAIN_REQUIRED", "Closure requires an explicit exhaustive-domain claim at the stated granularity.")
    ...
    if snapshot.epistemic_standing in {EpistemicStanding.UNKNOWN, EpistemicStanding.DERIVED}:
        return ResolutionOperationResult(snapshot, False, "EPISTEMIC_STANDING_INSUFFICIENT", "Derived or unknown standing cannot be promoted to closure without external resolution evidence.")
    event = AnswerSpaceHistoryEvent(
    ...
    updated = replace(snapshot, resolution_state=ResolutionState.CLOSED, history=snapshot.history + (event,))
